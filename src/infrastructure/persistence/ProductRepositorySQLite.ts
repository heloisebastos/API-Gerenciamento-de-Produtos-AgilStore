...
export class SqliteProductRepository implements ProductRepository {
    ...
    async findByName(name: string): Promise<Product | null> { return new Promise((resolve, reject) => { const query = `SELECT * FROM products WHERE name = ?`; db.get(query, [name], (err, row) => { if (err) { reject(err); } else { resolve(row as Product || null); } }); }); } async findByPartialName(partialName: string): Promise<Product[]> { return new Promise((resolve, reject) => { const query = `SELECT * FROM products WHERE name LIKE ?`; db.all(query, [`%${partialName}%`], (err, rows) => { if (err) { reject(err); } else { resolve(rows as Product[]); } }); }); }
